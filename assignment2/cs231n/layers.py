...
def batchnorm_backward_alt(dout, cache):
  ...
  dbeta = np.sum(dout, axis=0)
  dx_hat = dout * gamma
  dvar = (dx_hat * (x - sample_mean) * -0.5 * np.power(sample_var + eps, -1.5)).sum(axis=0)
  dmean = (dx_hat * np.power(sample_var + eps, -0.5)).sum(axis=0)
  dmean += (dvar * 2 * (x - sample_mean)  / m).sum(axis=0)
  dx = dx_hat * np.power(sample_var + eps, -0.5) + dvar*2*(x - sample_mean) / m + dmean * -1 / m
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def conv_backward_naive(dout, cache):
  ...
  for i in range(H_out):
  	for j in range(W_out):
  		...
  		# 注意这里dw和dx_pad的求法，从上面前向的计算来看，对于每个x_pad，它都参与了与每个w的核的相乘，反观对于w的每个核都是与同一个x_pad的相乘
  		# 这就导致了求dw的时候只需要一个x_pad，然而求每个d_pad时却需要所有的w，就是如下的求法
  		for k in range(F):
  			# use the shape to write the code if you aren't so understand

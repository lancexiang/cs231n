...
def rnn_backward(dh, cache):
  ...
  pass
  N, T, H = dh.shape
  D = cache[0][1].shape[0] # (next_h, Wx, ..)
  stepdprev_h = np.zeros((N, H))
  dx, dWx, dWh, db = [], np.zeros((D, H)), np.zeros((H, H)), np.zeros(H)
  for t in range(T-1, -1, -1):
    stepdx, stepdprev_h, stepdWx, stepWh, stepdb = rnn_step_backward(dh[:, t, :] + stepdprev_h, cache[t])
    dx.append(stepdx)
    dWx, dWh, db = dWx + stepdWx, dWh + stepWh, db + stepdb
  dh0 = stepdprev_h
...
def sigmoid(x):
  ...
  sigmoid(x) = 1/(1+exp(-x)), maybe the code provided can be fast, but no wrong.
  """
  pos_mask = (x >= 0)
  neg_mask = (x < 0)
  z = np.zeros_like(x)
